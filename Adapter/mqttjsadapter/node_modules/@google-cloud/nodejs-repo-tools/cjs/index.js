...
exports.run = (cmd, cwd) => {
  const output = childProcess.execSync(cmd, { cwd: cwd, shell: true }).toString().trim();
  try {
...
exports.spawnAsyncWithIO = (cmd, args, cwd, debug) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('error', finish).on('close', () => {
      finish();
    }).on('exit', () => {
      finish();
    });
...
class Try {
  ...
  execute() {
    ...
    return Promise.resolve().then(() => this._test(assert)).then(() => this.resolve()).catch(err => {
      this._error = err;
      this._iteration++;
      this._delay = Math.min(this._delay * this._multiplier, this._maxDelay);
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          Promise.resolve().then(() => this.execute()).then(resolve, reject);
        }, this._delay);
      });
    });
  }
